...
import ch09.classpath.WildcardEntry;
...
public class ClassPath {
   ...
   public ch09.classpath.ClassPath parse(String jreOption, String cpOption) {
        try {
            ch09.classpath.ClassPath cp = new ch09.classpath.ClassPath();
    ...
    public byte[] ReadClass(String className) {
        ...
        try {
            if ((data = bootClassPath.readClass(className)).length > 0) {
               // System_.out.printf("[Loaded %s from bootClassPath]\n",className);
                return data;
            }else{
                throw new Exception();
            }
            ...
            try {
                if ((data = extClassPath.readClass(className)).length > 0) {
                   //System_.out.printf("[Loaded %s from extClassPath]\n",className);
                    return data;
                }else{
                    throw new Exception();
                }
                ...
                try {
                    if ((data = userClassPath.readClass(className)).length > 0) {
                      //  System_.out.printf("[Loaded %s from userClassPath]\n",className);
                        return data;
                    }
                } catch (Exception eee) {
                    return new byte[0];
