package ch08.rtda.heap.constant_pool;
...
public class ConstantPool {
    public Class clazz;
    public ch08.rtda.heap.constant_pool.Constant[] consts;

    public static ConstantPool newConstantPool(Class clazz, ch08.classfile.ConstantPool cfCp) {
        int cpCount = cfCp.constantPool.length;
        ch08.rtda.heap.constant_pool.Constant[] consts = new ch08.rtda.heap.constant_pool.Constant[cpCount];
        ConstantPool constantPool = new ConstantPool();
        constantPool.clazz = clazz;
        constantPool.consts = consts;
        for (int i = 1; i < cpCount; i++) {
            ConstantInfo cpInfo = cfCp.constantPool[i];
            consts[i]=new ch08.rtda.heap.constant_pool.Constant();
    ...
    public ch08.rtda.heap.constant_pool.Constant getConstant(UInteger index){
