...
/*
    ...
    public static ch07.rtda.OperandStack newOperandStack(UInteger maxStack){
        if(maxStack==null){
            maxStack=UInteger.valueOf(0);
        }
        if(maxStack.intValue()>0){
            ch07.rtda.OperandStack operandStack=new ch07.rtda.OperandStack();
            operandStack.slots=new ch07.rtda.Slot[maxStack.intValue()];
            operandStack.size=UInteger.valueOf(0);

            for(int i=0;i<maxStack.intValue();i++){
                operandStack.slots[i]=new ch07.rtda.Slot();
            }
            return operandStack;
        }
        return null;
...
/*    func (self *OperandStack) PushInt(val int32) {
  ...
  /*  func (self *OperandStack) PushFloat(val float32) {
    ...
    public void pushSlot(ch07.rtda.Slot slot){
